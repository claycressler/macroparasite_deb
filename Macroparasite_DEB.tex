...
\begin{alltt}
\hlkwd{require}(deSolve)
\hlcom{## Mouse parameters}
\hlcom{##------------------}
\hlcom{## imax for balb/c = 7.86 g food/g mouse; black6 = 6.54 g food/g mouse}
\hlcom{## rho = 1 day}
\hlcom{## balb/c theta = 3.93 g lean/g fat; black6 = 3.5 g lean/g fat}
\hlcom{## uI = 20 days (for CD4 cell)}
\hlcom{## mC = 11 hours (half-life for neutrophils, 60-70% of standing immune defense; 60 day lifespan for lymphocytes, 40% of standing immune defense)}
pars <- \hlkwd{c}(imax=, theta=, rho=,
          epsA=, epsG=, epsR=, epsI=,
          Linf=34, alpha=1, gmin=25,
          m=, mc=, mi=,
          k=, b=, uI=,
          umin=, uacc=,
          epsP=, sigmaC= hC=,
...
\hlcom{## This model is slightly modified to focus on P_2 parasites whose biomass causes}
P2_deb <- \hlkwd{function}(t, y, p) \{
\hlcom{    ## HOST PARAMETERS}
\hlcom{    ## max. SA-specific ingest rate [food g^(-2/3) day^(-1)]}
    imax <- \hlkwd{unname}(p[\hlstr{"imax"}])
\hlcom{    ## target body condition [dimensionless ratio]}
    theta <- \hlkwd{unname}(p[\hlstr{"theta"}])
\hlcom{    # gut transit time [day^(-1)]}
    rho <- \hlkwd{unname}(p[\hlstr{"rho"}])
\hlcom{    ## biomass conversion efficiencies [dim.]}
    epsA <- \hlkwd{unname}(p[\hlstr{"epsA"}]) # ingestion to assimilation
    epsG <- \hlkwd{unname}(p[\hlstr{"epsG"}]) # assimilate to growth
    epsR <- \hlkwd{unname}(p[\hlstr{"epsR"}]) # assimilate to reserve
    epsI <- \hlkwd{unname}(p[\hlstr{"epsI"}]) # reserve to immune
\hlcom{    ## max structural length [mm]}
    Linf <- \hlkwd{unname}(p[\hlstr{"Linf"}])
\hlcom{    ## shape parameter (converts mm to g) [g mm^(-1)]}
    alpha <- \hlkwd{unname}(p[\hlstr{"alpha"}])
\hlcom{    ## growth rate for host in good condition [day^(-1)]}
    gmin <- \hlkwd{unname}(p[\hlstr{"gmin"}])
\hlcom{    ## maintenance rates for different tissues [day^(-1)]}
    m <- \hlkwd{unname}(p[\hlstr{"m"}])
    mc <- \hlkwd{unname}(p[\hlstr{"mc"}])
    mi <- \hlkwd{unname}(p[\hlstr{"mi"}])
\hlcom{    ## constitutive immune biomass per total weight [dim.]}
    k <- \hlkwd{unname}(p[\hlstr{"k"}])
\hlcom{    ## induced immune response rate [g^(-1) day^(-1)]}
    b <- \hlkwd{unname}(p[\hlstr{"b"}])
\hlcom{    ## biomass loss rate of induced immune response [day^(-1)]}
    uI <- \hlkwd{unname}(p[\hlstr{"uI"}])
\hlcom{    ## host background mortality rate [day^(-1)]}
    umin <- \hlkwd{unname}(p[\hlstr{"umin"}])
\hlcom{    ## starvation mortality [day^(-1)]}
    uacc <- \hlkwd{unname}(p[\hlstr{"uacc"}])
    epsAmin <- \hlkwd{unname}(p[\hlstr{"epsAmin"}])
    heps <- \hlkwd{unname}(p[\hlstr{"heps"}])
