...
png(file="Sarah_Fig2.png", height=4, width=8, units='in', res=300)
par(mar=c(5,5,1,1), oma=rep(0,4), mfrow=c(1,2))
plot.new()
plot.window(xlim=c(0.9,2.1), ylim=c(24, 35))
box('plot')
...
points(c(1,2), c(hiloresults[1,1], lohiresults[1,1]), type='l', lty=3, lwd=2)
mtext(side=2, line=3, "Body weight (g)", cex=1.5)
legend(x='bottomright', lty=1:3, c("Resistance", "Tolerance", "Both"), lwd=1.5, bty='n')
legend(x="topleft", "A", bty='n', cex=1.3)

plot.new()
plot.window(xlim=c(0.9,2.1), ylim=c(0, 40))
box('plot')
axis(1, at=c(1,2), labels=c("Pre-infection", "Post-infection"))
axis(2)
points(c(1,2), c(hiloresults[2,2]*100, lohiresults[2,2]*100), type='l', lwd=2)
points(c(1,2), c(hiloresults[2,3]*100, lohiresults[2,3]*100), type='l', lty=2, lwd=2)
points(c(1,2), c(hiloresults[2,1]*100, lohiresults[2,1]*100), type='l', lty=3, lwd=2)
mtext(side=1, line=-2, "Timing of diet supplement", cex=1.5, outer=TRUE)
mtext(side=2, line=3, "Parasite biomass (mg)", cex=1.5)
legend(x="topleft", "B", bty='n', cex=1.3)
dev.off()
